...
void heap_insert_tests(max_heap &hp) {
	...
	std::cout << "***My test cases*** without swap_up" << std::endl;
	hp.insert(text_item{"item", 18});
	std::cout << "Top of heap is: " << hp.top() << std::endl;
	hp.insert(text_item{"item", 15});
	std::cout << "Top of heap is: " << hp.top() << std::endl;
		
	// insert with a swap_up / multiple swap_ups
	std::cout << "***My test cases*** with swap_up" << std::endl;
	hp.insert(text_item{"item6", 18});
	std::cout << "Top of heap is: " << hp.top() << std::endl;
	hp.insert(text_item{"item7", 15});
	std::cout << "Top of heap is: " << hp.top() << std::endl;
		// <INSERT TEST(S) HERE>
}
...
void heap_delete_tests(max_heap &hp) {
	...
	//--- Specific insert functionality that should be tested:
	// Creating a new heap, {3, 2, 1}
	int testheap_size = 4;
	max_heap testheap1(testheap_size);
	std::cout << testheap1.empty() << std::endl;
	testheap1.insert(text_item{"item3", 9});
	testheap1.insert(text_item{"item2", 9});
	testheap1.insert(text_item{"item1", 9});

	// Creating a second heap, {3, 1, 2}
	max_heap testheap2(testheap_size);
	testheap2.insert(text_item{"item3", 9});
	testheap2.insert(text_item{"item1", 9});
	testheap2.insert(text_item{"item2", 9});
	// remove_max works when swap_down with left child
	std::cout << "*** TESTING delete_max works when swap_down with left child ***" << std::endl;
	std::cout << "test hp2 = {3, 2, 1}" << std::endl;
	testheap1.delete_max();
	std::cout << "top should be item2: " << testheap1.top() << std::endl;
	
	// remove_max workd when swap_down with right child
		// <INSERT TEST(S) HERE>
	std::cout << "*** TESTING delete_max works when swap_down with right child ***" << std::endl;
	std::cout << "test hp2 = {3, 1, 2}" << std::endl;
	testheap2.delete_max();
	std::cout << "top should be item2: " << testheap1.top() << std::endl;
	// remove_max on an empty heap (should throw exception similar to top())
	max_heap emptyheap = max_heap(1);
	emptyheap.insert(text_item{"item", 9});
	
	std::cout << "delete_max should throw error when heap is empty: " << std::endl;
	emptyheap.delete_max();
	try {
		text_item temp = emptyheap.top(); //should throw and exception without items in heap
		std::cout << "Top of heap is: " << temp << std::endl;	
