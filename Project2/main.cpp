...
void tree_tester(string_bst const &tree) {
	...
	//--- Specific word_frequency functionality that should be tested:
		// testing for word "function" in sample one
		if (tree.size() > 1) {
			string to_find = "function";
			int num_times = tree.word_frequency(to_find);
			std::cout << "Testing the word, function in sample1, frequency. Should be only be found once" << std::endl;
			std::cout << "Found: "<< to_find <<
				" in the input file " << num_times
				<< " time(s)." << std::endl;
		}
		// can search through both left and right subtrees if not found at current node
		// finding word "index" which is the first node of sample1
			if (tree.size() > 1) {
			string to_find = "index";
			cout << "Is this the first node of the tree? " <<
		(tree.get_root()->data.word.compare(to_find) == 0) << std::endl;
			int num_times = tree.word_frequency(to_find);
			std::cout << "Found: "<< to_find <<
				" in the input file " << num_times
				<< " time(s)." << std::endl;
		...
		if (tree.size() > 1) {
			string to_find = "computer";
			int num_times = tree.word_frequency(to_find);
			std::cout << "Testing the word, computer in sample1, frequency. Should not be found, produce 0" << std::endl;
			std::cout << "Found: "<< to_find <<
				" in the input file " << num_times
				<< " time(s)." << std::endl;
		}
